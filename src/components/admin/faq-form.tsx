...
export function FaqForm({
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const url = isNew ? "/api/faqs" : `/api/faqs/${form.id}`;
      ...
      if (isNew) router.push(`/admin/faqs/${json.data.id}`);
      else router.refresh();
    } catch (e) {
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        <Section title="Question & Answer">
          <Field label="Question *" hint="Ask it the way a client would.">
            <Input
              value={form.question}
          ...
          <Field
            label="Answer *"
            hint="Markdown supported. Keep it concise — 2–4 sentences reads best."
          >
            <Textarea
              value={form.answer}
              onChange={(e) => set("answer", e.target.value)}
              rows={8}
              placeholder="Wedding dates typically book 6–12 months ahead…"
            />
          </Field>
          <Field label="Category">
            <Input
              ...
              placeholder="Booking · Logistics · Gear · Deliverables"
            />
          </Field>
        </Section>
      </div>
